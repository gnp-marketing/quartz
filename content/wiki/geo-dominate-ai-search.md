...
## 四、IF-GEO 2026：多查詢衝突融合

### 論文基本資訊

- **標題**：IF-GEO: Conflict-Aware Instruction Fusion for Multi-Query Generative Engine Optimization
- **作者**：Heyang Zhou、JiaJia Chen、Xiaolu Chen、Jie Bao、Zhen Chen、Yong Liao
- **機構**：中國科學技術大學 + 合肥綜合國家科學中心
- **發表時間**：2026-01-20（arXiv:2601.13938v1）

### 核心問題

前三篇論文的 GEO 方法（手工規則、AutoGEO、AgentGEO）都預設**單一查詢**優化場景，但現實中：
- 一份文件同時被數十種不同意圖的查詢找到
- 針對不同查詢的修改指令往往互斥（「展開說明」vs「保持簡短」）或競爭同一段落空間

診斷實驗確認：針對單查詢調優後，69.2% 的非目標查詢出現相對退步，非目標查詢的負增益率高達 30.6%。

### 框架：diverge-then-converge

**Phase I（發散）**：
1. Query Mining：從文件反推加權代表查詢集（N=5 為最優）
2. Request Generation：針對每個查詢生成結構化修改請求（錨點 + 建議 + 必要性分）

**Phase II（收斂）**：
3. 優先化與去重：全域優先分 = 查詢普及度 × 必要性分
4. 衝突解決：互斥指令擇一保留或生成折衷指令
5. 藍圖建構：輸出 JSON 全域修訂藍圖
6. 藍圖導向修訂：嚴格按藍圖逐段執行，未提及段落原文保留

### 核心成果

| 方法 | Obj. Overall | WCP↑ | WTR↑ | DR↓ |
|------|-------------|------|------|-----|
| Auto-GEO | 7.59 | -0.0511 | 73.56% | 0.0043 |
| AgentGEO（CR 引用率 79.52%） | — | — | — | — |
| **IF-GEO** | **11.03** | **-0.0090** | **80.50%** | **0.0023** |

Gemini-2.0-Flash 跨引擎泛化：IF-GEO 仍最強（14.17 Mean，84.07% WTR）。

### 新評估指標：風險感知穩定性

- **WCP（最差查詢表現）**：min(Δv_i)，安全下界
- **DR（下行風險）**：負增益的平方均值，只懲罰退步
- **WTR（勝平率）**：不退步查詢的比例

詳見 [[geo-risk-aware-stability-metrics]]，完整框架見 [[ifgeo-framework]]。
...
### IF-GEO 2026 論文延伸
- [[ifgeo-framework]] — IF-GEO 多查詢衝突融合框架（完整技術說明）
- [[geo-multi-query-optimization]] — 多查詢競爭衝突問題定義與診斷數據
- [[geo-risk-aware-stability-metrics]] — WCP / DR / WTR 風險感知穩定性指標
